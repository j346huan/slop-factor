...
def test_resolves_exact_manual_and_issue_dates() -> None:
    ...
    issue = resolve_scan_parameters(
        event_name="issues",
        issue_title="Scan request: 2099-01-01",
        today=date(2099, 1, 3),
    )

    assert manual.scan_date == date(2099, 1, 2)
    assert manual.max_results == 75
    assert issue.scan_date == date(2099, 1, 1)
...
@pytest.mark.parametrize(
    ("event_name", "issue_title", "manual_date", "max_results"),
    [
        ("issues", "Please scan yesterday", "", "50"),
        ("workflow_dispatch", "", "2099-01-04", "50"),
        ("workflow_dispatch", "", "", "0"),
    ],
)
def test_rejects_invalid_or_future_scan_requests(
    event_name: str, issue_title: str, manual_date: str, max_results: str
) -> None:
